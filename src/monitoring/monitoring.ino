...
// // Pin assignments
// #define I2C_SDA  4   // INPUT  Data pin
// #define I2C_SCL  5   // INPUT  Clock pin
// #define PWR_GOOD 40   // INPUT  Power Good
// #define PWR_BAD  41   // OUTPUT Power Bad

// // I2C input address
// #define TELEM_V12_ADDR 0x40 
// #define TELEM_V5_ADDR  0x41
// #define TELEM_V3_ADDR  0x44
...
Adafruit_INA219 inaC(0x44);

// Your actual shunt resistor in milliohms — measure with a multimeter to confirm
const float SHUNT_MOHM = 50.0; // 50mΩ = 0.05Ω (common on cheap modules)

// I = V_shunt / R_shunt → mA = mV / Ω = mV / (mΩ/1000)
float getCurrent_mA_corrected(Adafruit_INA219 &ina) {
  return ina.getShuntVoltage_mV() / (SHUNT_MOHM / 1000.0);
}
...
void setup() {
  ...
  if (!inaB.begin()) Serial.println("INA219 (0x41) NOT FOUND!");
  if (!inaC.begin()) Serial.println("INA219 (0x44) NOT FOUND!");

  // Use 32V_1A just to initialize the chip — calibration doesn't 
  // matter since we're reading shunt voltage directly
  inaA.setCalibration_32V_1A();
  inaB.setCalibration_32V_1A();
  inaC.setCalibration_32V_1A();
...
void loop() {
  ...
  Serial.printf("0x40  (12v): V=%.3f V, I=%.3f mA\n", inaA.getBusVoltage_V(), getCurrent_mA_corrected(inaA));
  Serial.printf("0x41   (5v): V=%.3f V, I=%.3f mA\n", inaB.getBusVoltage_V(), getCurrent_mA_corrected(inaB));
  Serial.printf("0x44 (3.3v): V=%.3f V, I=%.3f mA\n", inaC.getBusVoltage_V(), getCurrent_mA_corrected(inaC));
